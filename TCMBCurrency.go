...
type currency struct {
	...
	BanknoteBuying  string `xml:"BanknotBuying"`
	...
	if err != nil {
		...
		if resp.StatusCode != http.StatusNotFound {
			...
			for i, curr := range tarih.Currency {
				...
				currDay.Currencies[i].BanknotBuying, err = strconv.ParseFloat(curr.BanknoteBuying, 64)
				...
				currDay.Currencies[i].Unit, err = strconv.Atoi(curr.CurrencyCode)
			}
			//Saves the data retrieved from the TCMB to the file in Json format
			SaveJSON(time.Now().Format("01-02-2006")+" - Currency.json", currDay)
		} else {
			currDay = nil
		}
	}
...
func SaveJSON(fileName string, key interface{}) {
	// Checks if there is a file with the same name inside. Otherwise, it creates a new file, if there is, it adds the data to the file.
...
func main() {
	...
	//Calculates how long it takes for data to arrive
